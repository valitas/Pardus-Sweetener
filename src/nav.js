...
const LOCATION_LINKS = {
  planet: [
    ...
    { key: "navHackLink", text: "Hack", url: "hack.php" },
    { key: "navShipyardLink", text: "Shipyard", url: "shipyard.php" },
    { key: "navBountyBoardLink", text: "Bounty Board", url: "bounties.php" },
  ],
  starbase: [
    {
      key: "navWeaponLink",
    ...
    { key: "navShipyardLink", text: "Shipyard", url: "shipyard.php" },
    { key: "navBountyBoardLink", text: "Bounty Board", url: "bounties.php" },
    { key: "navFlyCloseLink", text: "Fly Close", url: "main.php?entersb=1" },
  ],
  building: [{ key: "navHackLink", text: "Hack", url: "hack.php" }],
};
...
function start() {
  ...
  shiplinks = new ShipLinks("table/tbody/tr/td[position() = 2]/a", matchShipId);
...
function applyConfiguration() {
  if (configured) {
    ...
    navTilesXEval = document.createExpression("tbody/tr/td", null);
    highlightedTiles = [];
...
function onGameMessage(event) {
  ...
  shiplinks.setContainer(document.getElementById("otherships_content"));
  shiplinks.update(config.navShipLinks);
...
function updateLocationLinks() {
  ...
  cbox = document.getElementById("commands_content");
  ...
  // building" anchor.
  if ((anchor = document.getElementById("aCmdPlanet"))) {
    loctype = "planet";
  } else if ((anchor = document.getElementById("aCmdStarbase"))) {
    loctype = "starbase";
  } else if ((anchor = document.getElementById("aCmdBuilding"))) {
    loctype = "building";
  } else {
    return;
  }
...
function matchShipId(url) {
  ...
  rx =
    /^javascript:scanId\((\d+),(?: |%20)*['"]([^'"]+)['"]\)|main\.php\?scan_details=(\d+)&scan_type=([A-Za-z]+).*$/;
  m = rx.exec(url);
  if (m) {
    var id = m[1];
    if (id) {
    ...
    if (minimap && minimapSector && minimapSector.sector == sectorName) {
      ...
      if (sector) {
        ...
        chrome.runtime.sendMessage(
          { target: "worker", requestMap: sectorName },
          configureMinimap,
        );
      }
    }
...
function configureMinimap(sector) {
  ...
  if (config.miniMapPlacement == "statusbox") {
    ...
    config.miniMapPlacement == "navbox" ||
    config.miniMapPlacement == "navboxXL"
  ) {
    // Add the map at the under the navigation box.
    // This allows for the biggest navigation area.

    var td = document.getElementById("tdSpaceChart");
    ...
    if (config.miniMapPlacement == "navbox") {
      size = document.getElementById("navarea").offsetWidth;
    } else {
      size = td.offsetWidth;
    }
    ...
    var td = document.getElementById("tdTabsRight");
...
function getCurrentSectorName() {
  var elt = document.getElementById("sector");
...
function updatePathfinding() {
  ...
  var xpr = navTilesXEval.evaluate(
    ...
    td;
  while ((td = xpr.iterateNext())) navidx[td.id] = td;
...
function updateNavigationGrid() {
  ...
  if (!config.navigationCoordinates) {
    ...
    Array.from(document.getElementsByClassName("coordGrid")).forEach((f) =>
      f.remove(),
    );
  ...
  var navAreaDimensions = [
    _trs[3].children.length,
    navtable.getElementsByTagName("tr").length,
  ];
  //in case people play on sizes other than 64
  //tile sizes are nf = 64, nf96 = 96, nf128 = 128
  var navTileSize = { nf: 64, nf96: 96, nf128: 128 }[
    _trs[3].children[0].getElementsByTagName("img")[0].className
  ];
...
function showpath(event) {
  ...
  while (cell && cell.nodeName != "TD") cell = cell.parentElement;
  if (!cell) return;
  ...
  if (cell.classList.contains("navImpassable")) return;
  ...
  for (
    var i = 0, end = Math.max(Math.abs(selectedx), Math.abs(selectedy));
    i < end;
    i++
  ) {
    if (i < Math.min(Math.abs(selectedx), Math.abs(selectedy)))
      n += -Math.sign(selectedy) * navSizeHor + Math.sign(selectedx);
    else if (i > Math.abs(selectedy)) n += Math.sign(selectedx);
    else if (i > Math.abs(selectedx)) n += -Math.sign(selectedy) * navSizeHor;
...
function addDrugTimer() {
  ...
  if (!useform || !useform.elements.resid || useform.elements.resid.value != 51)
    return;

  let usebtn = useform.elements.useres;
  if (!usebtn || usebtn.dataset.pardusSweetener) return;
...
function displayDrugTimer(ukey, usebtn, data) {
  ...
  function getStimClearTime(now, ukey, data) {
    var t = Math.floor(data[ukey + "stimTimerClear"] / 1000);
    if (t > now) {
      timerDiv.appendChild(document.createElement("br"));
      timerDiv.appendChild(document.createTextNode("Stim free in:"));
      timerDiv.appendChild(document.createElement("br"));
      diff = getTimeDiff(data[ukey + "stimTimerClear"], Date.now());
      timerDiv.appendChild(
        document.createTextNode(
          diff["hr"] + "h" + diff["min"] + "m" + diff["sec"] + "s",
        ),
      );
  ...
  var drugestimation = document.createElement("span");
  drugestimation.id = "drugestimate";
  drugestimation.textContent = "0";
  timerDiv.appendChild(document.createTextNode("Estimate: "));
  timerDiv.appendChild(drugestimation);
  timerDiv.appendChild(document.createTextNode(" APs"));
  let doctorpredict = 8;
  ...
  chrome.storage.local.get(
    [universe + "advSkills", ukey + "doctor"],
    getAdvSkills.bind(this, universe, ukey),
  );
  ...
  drugusefield.addEventListener("input", (event) => {
    ...
    drugginess +=
      Math.max(0, diff["hr"]) + (diff["min"] > -1 || diff["sec"] > -1 ? 1 : 0);
  ...
  if (!data[ukey + "drugTimerClear"]) {
    ...
    timerDiv.appendChild(document.createTextNode("Drug used:"));
    timerDiv.appendChild(document.createElement("br"));

    diff = getTimeDiff(Date.now(), data[ukey + "drugTimerLast"]);
    timerDiv.appendChild(
      document.createTextNode(
        diff["hr"] + "h" + diff["min"] + "m" + diff["sec"] + "s ago",
      ),
    );
    timerDiv.appendChild(document.createElement("br"));
    ...
    if (data[ukey + "drugTimerClear"] > Date.now()) {
      ...
      timerDiv.appendChild(document.createElement("br"));
      diff = getTimeDiff(data[ukey + "drugTimerClear"], Date.now());
      timerDiv.appendChild(
        document.createTextNode(
          diff["hr"] + "h" + diff["min"] + "m" + diff["sec"] + "s",
        ),
...
function displayStimTimer(ukey, usebtn, data) {
  ...
  function getDrugClearTime(now, ukey, data) {
    var t = Math.floor(data[ukey + "drugTimerClear"] / 1000);
    if (t > now) {
      timerDiv.appendChild(document.createElement("br"));
      timerDiv.appendChild(document.createTextNode("Drug free in:"));
      timerDiv.appendChild(document.createElement("br"));
  ...
  if (!data[ukey + "stimTimerClear"]) {
    // No data, so make some nice comments
    timerDiv.appendChild(document.createTextNode("No stims used yet."));
  } else {
    // We have data, display current addiction
    timerDiv.appendChild(document.createTextNode("Stims used:"));
    timerDiv.appendChild(document.createElement("br"));

    diff = getTimeDiff(Date.now(), data[ukey + "stimTimerLast"]);
    timerDiv.appendChild(
      document.createTextNode(
        diff["hr"] + "h" + diff["min"] + "m" + diff["sec"] + "s ago",
      ),
    ...
    if (data[ukey + "stimTimerClear"] > Date.now()) {
      ...
      timerDiv.appendChild(document.createTextNode("You are unstimmed."));
    }
  }
}
...
function usedDrugs(useform, ukey) {
  ...
  if (amount > useform.childNodes[3].textContent.substring(1)) return;
  if (amount > 0) {
    chrome.storage.sync.get(
      [
        ukey + "drugTimerLast",
...
function usedDrugs2(amount, ukey, data) {
  ...
  var doctorCompensation = compensateDoctor(
    amount,
    ukey,
    data[ukey + "doctor"],
    data[ukey + "extraDrug"],
...
function usedStims(useform, ukey) {
  ...
  if (amount > 0) {
    ...
    // too bad!
    if (useform.elements.resid.value == 29) amount *= 2;
...
function getTimeDiff(time1, time2) {
  ...
  diff["min"] = Math.floor(
    ((Math.floor(time1 / 1000) - Math.floor(time2 / 1000)) % (60 * 60)) / 60,
  );
  diff["hr"] = Math.floor(
    ((Math.floor(time1 / 1000) - Math.floor(time2 / 1000)) % (60 * 60 * 24)) /
      3600,
  );
  diff["day"] = Math.floor(
    (Math.floor(time1 / 1000) - Math.floor(time2 / 1000)) / (60 * 60 * 24),
...
function updateRoutePlanner(data) {
  if (highlightedRPTiles) {
    ...
    for (let i = 0; i < highlightedRPTiles.length; i++) {
      highlightedRPTiles[i].setAttribute(
        "class",
        highlightedRPTiles[i]
          .getAttribute("class")
          .replace("sweetener-routeplanner", ""),
      );
    }
  }
  highlightedRPTiles = [];
  ...
  a.sort(function compare(a, b) {
    if (a.getAttribute("onclick") === null) return b;
    if (b.getAttribute("onclick") === null) return a;
    return (
      parseInt(a.getAttribute("onclick").split(/[()]/g)[1]) -
      parseInt(b.getAttribute("onclick").split(/[()]/g)[1])
    );
  });
  ...
  for (var j = 0; j < a.length; j++) {
    if (
      a[j].getAttribute("onclick") !== null &&
      idList.includes(parseInt(a[j].getAttribute("onclick").split(/[()]/g)[1]))
    ) {
      a[j].parentNode.setAttribute(
        "class",
        a[j].parentNode.getAttribute("class") + " sweetener-routeplanner",
      );
...
function highlightVisited(data) {
  ...
  for (var i = 0; i < a.length; i++) {
    if (!a[i].getAttribute("onclick")) continue;
    let loc = a[i].getAttribute("onclick").split(/\(|\)/g)[1];
    if (locs.includes(loc)) {
      let decayProportion =
        Math.round(
          (10 * (Date.now() - data[ukey + "visit"][loc])) / decayTime,
        ) / 10;
      let red = 0;
      let green = 255;
...
function showMissions(data) {
  ...
  if (document.getElementById("aCmdCleanWh")) {
    document
      .getElementById("aCmdCleanWh")
      .addEventListener(
        "click",
        Mission.removeMission.bind(null, data, userloc),
      );
  }
  ...
  function displayMissions(list, data) {
    ...
    var tr = t.appendChild(
      document.getElementById("cargo").firstChild.lastChild.cloneNode(true),
    );
    tr.firstChild.firstChild.setAttribute(
      "style",
    ...
    td.style =
      "background-image:url('//static.pardus.at/img/stdhq/panel.png');background-repeat:repeat-y;text-align:left;";
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      if (mission.locId) {
        if (
          Sector.getIdFromLocation(userloc) ===
          Sector.getIdFromLocation(mission.locId)
        ) {
          if (minimap) {
    ...
    btn.addEventListener(
      "click",
      Mission.clearMissionStorage.bind(
        null,
        function () {
    ...
    if (!document.getElementById("missionDisplayTable")) {
      document
        ...
        .parentNode.insertBefore(t, document.getElementById("cargo"));
    }
