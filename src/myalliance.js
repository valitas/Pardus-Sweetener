...
(function (doc, TreeSlicer) {
  var // The names of the config keys we use
    allianceQLsKey,
    allianceQLsEnabledKey,
    allianceQLsMTimeKey,
  ...
  function start() {
    ...
    // capitalised.
    universeName = universe.substr(0, 1).toUpperCase() + universe.substr(1);
  ...
  function processTabstyleTable(table, qls) {
    var slicer, match, text, name, qltext, offset, spans, child, icon, entry;
    ...
    var rx =
      /(?:\{\s*([A-Za-z0-9](?:[-+_/#&()A-Za-z0-9 \t\u00a0]{0,58}[A-Za-z0-9)])?)\s*\}|\[\s*([A-Za-z0-9](?:[-+_/#&()A-Za-z0-9 \t\u00a0]{0,58}[A-Za-z0-9)])?)\s*\]|^\s*?([A-Z](?:[-+_/#&()A-Za-z0-9 \t\u00a0]{0,58}[A-Za-z0-9)])?)(?:\s*:)?)?\s*((?:d|r(?:\d*)?);m?;t?;r?;[efn]{0,3};[feun]{0,4};b?;[f:0-9\s]*;[e:0-9\s]*;[u:0-9\s]*;[n:0-9\s]*;[gl:0-9\s]*;[0-6\s]*;[0-9,\s]*;[0-9,\s]*;[fn\s]*;[feun\s]*;[gl:0-9\s]*;[0-6\s]*;[0-9,\s]*;[0-9,\s]*;[0-9\s]*[0-9](?:\s*;\s*[es])?)/gm;
