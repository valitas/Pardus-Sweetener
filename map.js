...
PSMap.prototype = {
  ...
  configure: function(sector, maxPixelSize) {
    ...
    if(tiles.length != cols*rows)
      throw new Error("Tile data and map dimensions don't match");

    if(cols > rows)
      // This is a wide map. We use cols to determine size. Height
  ...
  // Just gets the 2D context of the canvas. You'll want this to clear
  // the map and mark tiles.
  get2DContext: function() {
    return this.canvas.getContext('2d');
  },

  // This "clears" the canvas, restoring the sector map. So this
  // effectively draws the sector map. The idea being: you'll want to
  // clear, then overlay dynamic stuff on the "background" map.
  clear: function(ctx) {
  ...
  markTile: function(ctx, col, row, style) {
    var grid = this.grid, size = this.tileSize, gstep = grid ? size+1 : size,
        x = col*gstep, y = row*gstep;

    // If the tiles are large enough, make the mark smaller so
    // the background shows a bit, let you know what type of tile the
    // marker is on.
    if(size > 10) {
      x += 2;
      y += 2;
  ...
  // Convert pixel x,y coordinates on the canvas to map row,col.  For
  // this purpose, if the map has a grid, points on the grid are
  ...
  setupBgCanvas: function() {
    ...
    if(grid) {
      // When the grid is enabled, we paint tiles of side size+1. The
      // extra pixel is really part of the grid line, but painting in
      // the tile colour first makes the map prettier.
      size += 1;

      // Since there is one less grid line than there are rows (or
      // columns), one of these "tile plus grid pixel" areas has to be
      // 1px smaller.  We feel it looks better if this is the first
      // row and the first column.  So we paint 1px up and to the
      // left, and let the canvas clip it.
      px0 = -1;
    }
    else
      px0 = 0;
  ...
  // of pixels.  Our tiles are square, so we only really compute this
  // for one dimension.
  //
  // Our tiles are of uniform size. This means we don't really
  // output a map of the requested dimensions, but the largest size we
  // can create, while keeping our cells square and uniform size, that
  // is still less than or equal than the specified pixel size.
  //
  // We want thin 1px grid lines if the tiles are big enough. When the
  // map is so large that the tiles become tiny, we don't want to
  // waste pixels in those.
  computeGrid: function(tiles, maxPixels) {
    ...
    // A tile would be size-1 pixels per side, the extra pixel is for
    // the grid. All our tiles fit in the allowed pixels because
    // there is one less grid line than there are tiles.
    if(size < 4) {
      // This means our tiles would be 2 pixels per side. We don't
      // want grid lines in this case.
